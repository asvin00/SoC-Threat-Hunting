...
### 1. **Splunk to External Systems**
- Webhooks for incident management (ServiceNow, Jira)
- Email alerts for critical events
- Syslog forwarding for SOAR platforms
...
### 3. **Automation**
- Python scripts for scheduled scans
- Event-triggered Python automation
- Database integrations for correlation
...
- Forwarder → Splunk: TLS 1.2 encrypted
- Splunk Web UI: HTTPS with self-signed certificates
- Internal communication: Host-only adapter (isolated)

### Access Control
- Splunk admin credentials: admin/Splunk@123
- Windows admin: admin/Windows@123
- Ubuntu admin: socadmin/Splunk@123
- All communications within 192.168.56.0/24 network

---

## Scalability & Future Enhancements

### Current Setup (Single Lab)
- 1 Splunk Indexer
- 1 Windows Endpoint
- 1 Attack simulation VM
  ...
  Splunk Forwarders → 
    Splunk Heavy Forwarders → 
      Splunk Indexer Cluster → 
        Splunk Search Heads (UI) → 
          External Systems (SOAR, SIEM, etc.)
```

### Potential Additions
- Linux endpoints (Apache, MySQL logs)
- Network devices (Firewall, Router logs)
